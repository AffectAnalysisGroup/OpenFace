		...
		while (!rgb_image.empty())
		{
			...
			for (size_t model = 0; model < face_models.size(); ++model)
			{
				...
				if (active_models[model])
				{
					...
					auto temp = face_analyser.GetCurrentAUsClass();
					open_face_rec.SetObservationActionUnits(face_analyser.GetCurrentAUsReg(), std::get<0>(temp), std::get<1>(temp));//face_analyser.GetCurrentAUsClass());
