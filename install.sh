...
dzdo apt-get -y update

if [[ `lsb_release -rs` != "18.04" ]]
  then   
    echo "Adding ppa:ubuntu-toolchain-r/test apt-repository "
    dzdo add-apt-repository ppa:ubuntu-toolchain-r/test -y
    dzdo apt-get -y update
fi

dzdo apt-get -y install build-essential
dzdo apt-get -y install gcc-8 g++-8

# Ubuntu 16.04 does not have newest CMake so need to build it manually
if [[ `lsb_release -rs` != "18.04" ]]; then   
  dzdo apt-get --purge remove cmake-qt-gui -y
  dzdo apt-get --purge remove cmake -y
  mkdir -p cmake_tmp
  cd cmake_tmp
  ...
  dzdo apt-get -y install cmake
fi

dzdo apt-get -y install zip
dzdo apt-get -y install libopenblas-dev liblapack-dev
dzdo apt-get -y install libgtk2.0-dev pkg-config libavcodec-dev libavformat-dev
dzdo apt-get -y install libtbb2 libjpeg-dev libpng-dev libtiff-dev libdc1394-22-dev
