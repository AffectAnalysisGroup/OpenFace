...
void FaceAnalyser::PredictStaticAUsAndComputeFeatures(const cv::Mat& frame, const cv::Mat_<float>& detected_landmarks)
{
	...
	//auto AU_Predictions_occurence, AU_distances_occurence;
	//auto [AU_predictions_occurence, AU_distances_occurence] = PredictCurrentAUsClass(orientation_to_use);
	auto AU_occurence_distance_tuple = PredictCurrentAUsClass(orientation_to_use);
	//auto AU_predictions_occurence, AU_distances_occurence = tie(temp)
	...
	//std::cout << "datatype of tuple 0th element " << typeid(std::get<0>(AU_occurence_distance_tuple)) << endl;
	//AU_predictions_class = AU_predictions_occurence;
	//AU_predictions_distances = AU_distances_occurence;
	AU_predictions_class = std::get<0>(AU_occurence_distance_tuple);
	AU_predictions_distances = std::get<1>(AU_occurence_distance_tuple);
	...
	for (size_t au = 0; au < AU_predictions_class.size(); ++au)
	{
		...
		if(success)
		{
			AU_predictions_class_all_hist[AU_predictions_class[au].first].push_back(AU_predictions_class[au].second);
	    		AU_predictions_distances_all_hist[AU_predictions_distances[au].first].push_back(AU_predictions_distances[au].second);
		}
		else
		{
			AU_predictions_class_all_hist[AU_predictions_class[au].first].push_back(0);
			AU_predictions_distances_all_hist[AU_predictions_distances[au].first].push_back(0);

			// Also invalidate AU if not successful
			AU_predictions_class[au].second = 0;
			AU_predictions_distances[au].second = 0;
		}
	}	
	...
	if(!postprocessed)
	{
		...
		while(all_ind < all_frames_size && success_ind < max_init_frames)
		{
			...
			if(valid_preds[all_ind])
			{
				...
				auto AU_predictions_distance_tuple = PredictCurrentAUsClass(views[success_ind]);
				// check datatype here
				auto AU_predictions_class = std::get<0>(AU_predictions_distance_tuple);
				auto AU_predictions_distance = std::get<1>(AU_predictions_distance_tuple);
				...
				for (size_t au = 0; au < AU_predictions_class.size(); ++au)
				{
					...
					// Added for distance
					AU_predictions_distances_all_hist[AU_predictions_distance[au].first][all_ind] = AU_predictions_distance[au].second;
...
void FaceAnalyser::ExtractAllPredictionsOfflineClass(std::vector<std::pair<std::string, std::vector<double>>>& au_distances, std::vector<std::pair<std::string, std::vector<double>>>& au_predictions,
	std::vector<double>& confidences, std::vector<bool>& successes, std::vector<double>& timestamps, bool dynamic)
{
	if (dynamic)
	{
...
std::tuple<std::vector<std::pair<std::string, double>>, std::vector<std::pair<std::string, double>>> FaceAnalyser::PredictCurrentAUsClass(int view)
{
	...
	if(!hog_desc_frame.empty())
	{
		...
		// Added distances vector
		std::vector<double> svm_lin_stat_distances;
		// Added distance
		AU_SVM_static_appearance_lin.Predict(svm_lin_stat_distances, svm_lin_stat_preds, svm_lin_stat_aus, hog_desc_frame, geom_descriptor_frame);
		...
		for(size_t i = 0; i < svm_lin_stat_aus.size(); ++i)
		{
			...
			// Added distance
			distances.push_back(std::pair<std::string, double>(svm_lin_stat_aus[i], svm_lin_stat_distances[i]));
		}

		std::vector<std::string> svm_lin_dyn_aus;
		std::vector<double> svm_lin_dyn_preds;
		// Added distances vector
		std::vector<double> svm_lin_dyn_distances;

		AU_SVM_dynamic_appearance_lin.Predict(svm_lin_dyn_distances, svm_lin_dyn_preds, svm_lin_dyn_aus, hog_desc_frame, geom_descriptor_frame, this->hog_desc_median, this->geom_descriptor_median);
		...
		for(size_t i = 0; i < svm_lin_dyn_aus.size(); ++i)
		{
			...
			// Added distance
			distances.push_back(std::pair<std::string, double >(svm_lin_dyn_aus[i], svm_lin_dyn_distances[i]));
		}
		
	}

	return make_tuple(predictions, distances);
}

std::tuple<std::vector<std::pair<std::string, double>>, std::vector<std::pair<std::string, double>>> FaceAnalyser::GetCurrentAUsClass() const
{
	return make_tuple(AU_predictions_class, AU_predictions_distances);
}
...
void FaceAnalyser::PostprocessOutputFile(std::string output_file)
{
	...
	std::vector<std::pair<std::string, std::vector<double>>> predictions_distances;
	...
	ExtractAllPredictionsOfflineClass(predictions_distances, predictions_class, certainties, successes, timestamps, dynamic);

	int num_class = (int)predictions_class.size();
    	int num_dist = (int)predictions_distances.size();
	...
	// Added for distance--duplicate
	// std::vector<std::string> au_class_names = GetAUClassNames();
	// std::sort(au_class_names.begin(), au_class_names.end());
	std::vector<int> inds_dist;
	...
	for (std::string au_name : au_class_names)
	{
		...
		{
			if (au_name.compare(predictions_distances[i].first) == 0)
			{
				inds_dist.push_back(i);
				break;
			}
		}
	}
	// End of distance
	// TODO check how to write to the output file
